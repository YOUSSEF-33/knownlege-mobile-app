...
interface Assignment {
  ...
  dead_line: string;
  questions: Question[];
  students: { submitted_at: string | null }[];
}
...
export default function AssignmentContentScreen() {
  ...
  const fetchAssignmentDetails = useCallback(async () => {
    ...
    try {
      ...
      setIsSubmitted(assignmentData.students[0]?.submitted_at != null);
            ...
            <View style={styles.assignmentContainer}>
              ...
              {isSubmitted && (
                <Text style={styles.submittedMessage}>
                  Assignment submitted on: {new Date(assignment?.students[0]?.submitted_at ?? "").toLocaleString()}
                </Text>
              )}
              {renderQuestions()}
              {!isSubmitted && (
